...
public class PIDMecanumMoveTask implements RobotControl {
    ...
    public void execute() {
        ...
        if (targetDistance - currentDistance <= profile.movement.forwardStopDist){
            double pwr = 0.10 + (targetDistance - currentDistance)/profile.movement.forwardStopDist * 0.90;
            robot.mecanumDrive(pwr, targetAngle + posCorrection, headCorrection);
        }
        else{
            robot.mecanumDrive(1,targetAngle + posCorrection, headCorrection);
        }
