...
public class autoMethodsCopy implements Runnable{
    ...
    private int tempsleepT = 150;


    public autoMethodsCopy(DcMotor.RunMode runMode, HardwareMap importedMap) {
        ...
        distance = map.get(DistanceSensor.class, "distance");
        ...
        pidRotate = new PIDController(.01, .00001, 0.003);

        // Set PID proportional value to produce non-zero correction value when robot veers off
        // straight line. P value controls samhow sensitive the correction is.
        pidDrive = new PIDController(0.006, 0.0005, 0.005);

        pidStrafe = new PIDController(0.05, 0.05, 0.2);
    ...
    public final double wheelDiameter = 4.5; //in inches
    ...
    public boolean sleepTReq = false;
...
//    public Coordinate skystone = new Coordinate(-1, -1);//spot that a new skystone can be found
    ...
    public void locate(boolean blue, boolean sample, boolean parkWall) {
        ...
        if (parkWall) {
            bridge.setPoint(robotWidth + 5, FieldSide / 2);
        } else {
            bridge.setPoint(34, FieldSide / 2);
        }
        for (int i = 0; i < sampleSpots.length; i++) {
            if (blue) sampleSpots[i] = new Coordinate(55, 8 * i + 4);
            else sampleSpots[i] = new Coordinate(55, FieldSide - 4 - 8 * i);
        }
        if (blue) foundationLocation.setPoint(bridge.getX(), FieldSide - 20);
...
//        }
    ...
    public void moveStonesBack(int index){
        int init = index;
        int moves = 2;
        if(index + 2 > 5) {
            moves = 5 - index;
    ...
    public void sample(boolean Specialstone, boolean foundationPull, boolean dumpClose, int cycles) {
        ...
        for (int i = 0; i < strafeStones.length; i++) {
            ...
            if (blue) strafeStones[i].addY(16 + robotLength/2);
            ...
            if (!skyFound) {
                ...
                if (tfod != null) {
                    ...
                    if (updatedRecognitions != null) {
                        ...
                        armIn();
                        intake();
                        goTo(sampleSpots[stoneSpot] , 0.7, false, false, false);
                        backward(0.9, bridge.getX() - robot.getX());
                        intakeOff();
                        grip();

                        orient(0.8, homeAngle);
        ...
        if (skyFound) {
            ...
            if (foundationPull) {
                ...
                if (blue) rotate(0.7, -getProxy(-180));
                else rotate(0.7, getProxy(180));
                //pullFoundation(true);
                //start
                pullUp();
                armMid();
                backward(0.8, buildZone.getX() - robot.getX() - robotLength / 2);
                pullDown();
                sleepT(450);
                armOut();
                //sleepTReq = true;
                if(blue) strafeLeft(.8, 14);
                else strafeRight(.8, 14);
                forward(0.8, robot.getX() - bridge.getX());
                release();
                armMid();

                pidRotate.setPID(0.0003, 0.00003, 0);
                if(blue)rotate(0.6, getProxy(homeAngle));
                else rotate(0.6, -getProxy(homeAngle));
                ...
                //goTo(bridge, 0.8, true, false, false);
                //forward(1, bridge.getY() - robot.getY());
            } else {
                goTo(dumpLocation, 0.8, true, false, false);
                deposit();
        ...
        if (cycles >= 2 || Specialstone) {
            if (Specialstone) {
                if(!parkWall) {
                    ...
                    moveStonesBack(stoneSpot - 3);
                    intake();
                    forward(1, 12);
                    strafe(0.8, dumpLocation.getX() - robot.getX());
                    intakeOff();
                    grip();
                    pullReq = true;
                    ...
                    goTo(sampleSpots[stoneSpot - 3], 0.8, false, false, true);
                    backward(0.8, getNiche());
                    intakeOff();
                    grip();
                    orient(0.7, homeAngle);
                    pullReq = true;
                    goTo(dumpLocation, 0.7, true, false, false);
                }
            ...
            while(p < cycles){
                if (sampleSpots[p] != null) {
                    ...
                    if (p == cycles - 1) {
                        armOut();
                        backward(0.8, FieldSide - 18 - robot.getY());
                        release();
                        sleepT(200);
                        armIn();
                    }
        ...
        if (asap) {
            ...
            if (forwardIncreasesX) {
                if (xFirst) {
                    ...
                    else strafe(0.8, point.getY() - robot.getY());
                } else {
                    if (combo) combination(power, point.getY() - robot.getY());
                    else strafe(0.8, point.getY() - robot.getY());
                ...
                if (xFirst) {
                    ...
                    else strafe(0.8, robot.getY() - point.getY());
                ...
                if (xFirst) {
                    ...
                    else strafe(0.8, robot.getX() - point.getX());
                    ...
                    else strafe(0.8, point.getX() - robot.getX());
                }
            } else {
                if (xFirst) {
                    if (combo) combination(power, point.getX() - robot.getX());
    ...
    public void combination(double power, double distance) {
        ...
        turn(0.6, Range.clip(distance, -1, 1) * -90);
        drive(power, distance);
        turn(0.6, Range.clip(distance, -1, 1) * 90);
    }
    ...
    public void timeStrafeRight(double power, double time) {
        ...
        botLeft.setPower(-power);
        botRight.setPower(power);
        sleepT((long) (time * 1000));//don't execute further code until after the wheels are done turning
        //updateCoordinates();
        resetTicks();
    }

    public final void sleepT(long milliseconds) {
        try {
    ...
    public void forward(double power, double distance) {
        resetAngle();
        distance = Math.abs(distance);
        double angle = getHeading();//subject to change
        robot.add(xCovered(angle, distance), yCovered(angle, distance));
        pidDrive.reset();
