 ...
 **/
    ...
    private void orientTo(double targetPos, double power)
    {
        ...
        if(startingPos < 0)
        {
            startingPos += 360;//changes startingPos to the same position, but as a positive for ease of calculations
        }
        ...
        {
            distanceToMove = (startingPos - targetPos);
            CCW = true;
        }else
        {
            distanceToMove = (targetPos - startingPos);
        }
        if(distanceToMove > 180)
        {
            distanceToMove = 360 - distanceToMove;
        }
        if(CCW)//if the rotation needs to be counter clockwise, this will make distanceToMove a negative angle for the use in driveSystem.turn
        {
    ...
    {
        /*
        find slope of current point and target point
        convert slope to an angle
        orientTo(that angle, power);
        move using distance formula
         */
        ...
        orientTo(rotation, power);//rotate to ending rotation after having moved
    }
